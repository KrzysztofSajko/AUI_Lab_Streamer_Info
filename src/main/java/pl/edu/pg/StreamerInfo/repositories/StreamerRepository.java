...
public interface StreamerRepository extends JpaRepository<Streamer, Long> {
    ...
    @Query("select s from Streamer s join s.playedGames g join g.genre gen where gen.id = :genre")
    List<Streamer> findAllByGenreId(@Param("genre") Long genreId);

    @Query("select s from Streamer s join s.playedGames g join g.genre gen where s.id = :streamerId and gen.id = :genre")
